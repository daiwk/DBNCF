...
void DBNCF::pretrain_old_version(string dataset, bool reset) 
{
...
#pragma omp critical
	...
	//多线程的时候，不知为啥这个值一直不变。。。所以强制了。。。。各自给->xxx	
//						int hidden_M = hidden_layers[l]->M;
//						int hidden_K = hidden_layers[l]->K;
//						int hidden_F = hidden_layers[l]->F;
						...
						for (int j = 0; j < hidden_layers[l]->M; j++) {
							if (l == 0) {
								hidden_vs[l][j] = input_hs[j];
							}
							else {
								hidden_vs[l][j] = hidden_hs[l - 1][j];
							}
						}
				...
				if (i == 0) {
					...
					// 注意下标：此函数读rbm-h*-l，输出rbm-h*-l+1
#pragma omp critical
					hidden_layers[i - 1]->train_full(reset, i - 1); 
					printf("layer %d trained\n", i - 1); 
					int hidden_size = hidden_layers[i - 1]->M;
					printf("hidden_size: %d\n", hidden_size);

					for(int m = 0; m < hidden_size; m++) {

						double now_vb = hidden_layers[i - 1]->vb[m];
